...
from aiogram.types import Message, CallbackQuery, InlineKeyboardMarkup, \
    InlineKeyboardButton, ContentType, ReplyKeyboardMarkup, ReplyKeyboardRemove
...
async def process_settings(message: Message):
    ...
    markup.add(InlineKeyboardButton('+ Добавить категорию',
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict,
                                    state: FSMContext):
    """Выводит все товары конкретной категории"""
...
async def set_category_title_handler(message: Message, state: FSMContext):
    """Устанавливает имя новой категории"""
...
async def delete_category_handler(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        if 'category_index' in data.keys():
            ...
            db.query(
                'DELETE FROM products WHERE tag IN (SELECT title FROM '
                'categories WHERE idx=?)', (idx,))
...
async def process_add_product(message: Message):
    """Запускает стейт для добавление нового продукта"""
...
async def process_cancel(message: Message, state: FSMContext):
    """Обработка кнопки отмена на названии товара"""
...
async def process_title_back(message: Message, state: FSMContext):
    """Отмена заполнения названия"""
    await process_add_product(message)


@dp.message_handler(IsAdmin(), state=ProductState.title)
async def process_title(message: Message, state: FSMContext):
    """Запрос на ввод описание товара"""
    async with state.proxy() as data:
...
async def process_body_back(message: Message, state: FSMContext):
    """
    Запрашивает изменение названия товара с помощью
    кнопки НАЗАД с текстом back_message
    """
    await ProductState.title.set()
...
async def process_body(message: Message, state: FSMContext):
    """Принимает описание и запрашивает изображение к товару"""
    async with state.proxy() as data:
...
async def process_image_photo(message: Message, state: FSMContext):
    """Принимает изображение и запрашивает цену товара"""
...
async def process_image_url(message: Message, state: FSMContext):
    """
    Запрашивает изменение описания при кнопке НАЗАД с текстом back_message
    и проверяет, было ли прислано изображение
    """
...
async def process_price_invalid(message: Message, state: FSMContext):
    """
    Запрашивает изменение изображение товара и проверяет ввод цены на товар
    """
    if message.text == back_message:
...
async def process_price(message: Message, state: FSMContext):
    """
    Принимает цену товара и выводит весь товар, также запрашивает
    подтверждение о сохранение товара, отправляя клавиатуру
    """
    async with state.proxy() as data:
...
async def process_confirm_invalid(message: Message, state: FSMContext):
    """Уведомляет пользователя если он ввел не тот ответ на подтверждение"""
...
async def process_confirm_back(message: Message, state: FSMContext):
    """
    Запрашивает изменение цены на товар при нажатии кнопки
    НАЗАД с текстом back_message
    """
...
async def process_confirm(message: Message, state: FSMContext):
    """
    Записывает данные в базу данных, если администратор
    подтвердил добавление товара
    """
    async with state.proxy() as data:
...
async def delete_product_callback_handler(query: CallbackQuery, callback_data: dict):
    """Удаляет товар из базы данных (Инлайн кнопка под товаром)"""
...
async def show_products(m, products, category_idx):
    """Выводит все товары по конкретной категории"""
