...
from keyboards.default.markups import all_right_message, \
    cancel_message, \
    submit_markup
...
async def process_questions(message: Message):
    """
    Проверяет есть ли вопросы от пользователей, если есть,
    то выводит вопросы и кнопку ОТВЕТИТЬ
    """
...
async def process_answer(query: CallbackQuery, callback_data: dict,
                         state: FSMContext):
    """Принимает ответ админа по конкретному вопросу"""
...
async def process_send_answer(message: Message, state: FSMContext):
    """Если админ отменил ответ на вопрос"""
...
async def process_send_answer(message: Message, state: FSMContext):
    """Отправляет ответ пользователю и удаляет вопрос из базы данных"""
    async with state.proxy() as data:
