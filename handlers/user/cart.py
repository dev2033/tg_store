...
async def process_cart(message: Message, state: FSMContext):
    """
    Выводит корзину пользователю. Если корзина не пустая, то выводит
    все товары с инлайн клавиатурой (product_markup) для изменения количества
    конкретного товара.
    """
...
async def product_callback_handler(query: CallbackQuery, callback_data: dict,
                                   state: FSMContext):
    """Изменяет количество каждого товара | idx - количество"""
...
async def process_checkout(message: Message, state: FSMContext):
    """Обработка кнопки оформлении заказа"""
...
async def process_check_cart_invalid(message: Message):
    """Если пользователь дал не тот ответ"""
...
async def process_name_back(message: Message, state: FSMContext):
    """Если пользователь нажал кнопку НАЗАД, перебрасывает на проверку заказа"""
    await CheckoutState.check_cart.set()
    await checkout(message, state)
...
async def process_name(message: Message, state: FSMContext):
    """Запрашивает адрес у пользователя и заполняет стейт именем пользователя"""
    async with state.proxy() as data:
...
async def process_address_back(message: Message, state: FSMContext):
    """Если пользователь нажал кнопку НАЗАД, изменяет имя пользователя"""
    async with state.proxy() as data:
...
async def process_confirm(message: Message, state: FSMContext):
    """Запрашивает изменение адреса пользователя"""
...
async def process_confirm(message: Message, state: FSMContext):
    ...
    if enough_money:
        ...
        async with state.proxy() as data:
            ...
            products = [idx + '=' + str(quantity)
                        for idx, quantity in db.fetchall(
                            '''SELECT idx, quantity FROM cart WHERE cid=?''',
                        (cid,))]  # idx=quantity
        ...
        await message.answer('У вас недостаточно денег на счете. '
                             'Пополните баланс!',
                             reply_markup=markup)
